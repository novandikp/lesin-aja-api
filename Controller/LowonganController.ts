...
import { acceptLowongan, ajuanLowongan, getLowongan, getLowonganByTag, getPelamar, getStatusLowongan } from "../Service/LowonganService"
...
router.get("/status/:id",TeacherChecker,async (req:Request, res:Response,next:NextFunction) => {
  const data = await getStatusLowongan(req.params.id,req.context.idchild)
  if (!data) return next(new ErrorHandler(HTTPStatus.ERROR,"Apply telah dilakukan atau data tidak ditemukan"))
  return send(res,HTTPStatus.OK,{data:data,status:true,message:""})
})
